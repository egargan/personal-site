...
> Svelte does a fantastic job of making sure components are only re-rendered when they need to be.
> In the above example, each `<Todo/>` component is only re-created when the todo value changes, even if the object reference isn’t the same! It’s all a bit magic.
>
