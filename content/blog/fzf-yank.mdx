...
Starting with custom Rg command
example from the [FZF Vim plugin’s
docs](https://github.com/junegunn/fzf.vim/blob/master/doc/fzf-vim.txt#L328), let’s add <mark
class="green">an `--expect` argument</mark>.

```vimscript {1,3} green:/--expect=ctrl-y/
...
We use this <mark class="cyan">`sink*`</mark> property to tell FZF to use our custom sink function. At the moment, all it does is echo the the argument that FZF gives it.
