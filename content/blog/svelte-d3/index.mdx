---
title: Chart-building with D3 and Svelte 5
created: "2025-08-24"
updated: "2025-08-24"
tags: ["svelte", "info vis"]
readTimeMins: 10
status: draft
intro: Svelte 5's rendering and reactivity features combine beautifully with D3's data tools. In this post, you'll see how.
...
which gives us the tools to transform data into SVG visualisations.
...
rendered in the DOM before asking D3 to do its thing.
...
I'll talk more about the benefits further down, but first, let's see how it looks to do things _properly_.
...
Whenever <mark class="purple">`stat`</mark> changes, <mark class="cyan">`transfers.map((d) => ({ ...d, stat: d[stat] }))`</mark> is re-evaluated, returning the new dataset.

<mark class="red">`Tween.of`</mark> takes this new dataset, and generates _a sequence of datasets_ that smoothly transition from the last value to the current.
...
<aside>
  ...
  That's why we have to transform the transfers data to add a new `stat` property (<mark class="cyan">`transfers.map((d) => ({ ...d, stat: d[stat] }))`</mark>), containing the data point we care about in each record.
</aside>

Over a period of 400 milliseconds (a default value), the tween rapidly steps through this sequence, setting <mark class="green">`statData.current`</mark> with every step. <mark class="green">`statData.current`</mark> is a reactive value, so whenever it changes, the `<path>`s are redrawn for the current dataset.

These steps happen very quickly. So quickly that all we see is a smooth morph animation between two charts.

<mark class="orange">`maxStat`</mark> becomes a reactive tween as well, but this one's just a number that changes between the maximum value of the selected dataset.
